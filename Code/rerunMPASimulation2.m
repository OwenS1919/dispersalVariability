function simOutStruct = rerunMPASimulation2(simStruct, areaStruct, ...
...
% simOutStruct - same as the input argument, but with the objective
    % function stuff re-calculated because idfk my results from the other
    % night all match each other for no reason that I can understand
...
simOutStruct.meanLarvContObs = zeros(nSims, nReefs);
simOutStruct.stdLarvContObs = zeros(nSims, nReefs);
...
for s = 1:nSims
    ...
    % store the mean and standard deviation of the biomass and larval
    % contribution at each reef
    ...
    for t = 1:simStruct.tObs
        conMatsSeq{t} = areaStruct.conMats{simStruct.obsSeqs{s}(t)};
    end
    larvContMat = calcLarvalContrib(popMatObsLocal, specStruct.fecundVec, ...
        conMatsSeq);
    simOutStruct.meanLarvContObs(s, :) = mean(larvContMat, 2)';
    simOutStruct.stdLarvContObs(s, :) = std(larvContMat, [], 2)';
