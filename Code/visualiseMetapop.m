function tL = visualiseMetapop(popMat, tVec, conMat, horizInd)
% visualiseMetapop will visualise the trajectories of a set of
...
hold on
for r = 1:nReefs
    plot(tVec, totPop(r, :), 'color', getColour(r, nReefs), 'lineWidth', ...
        1.25)
    xlim([0, max(tVec) + 1])
end
...
for a = 1:min(4, nAges)
    ...
    for r = 1:nReefs
        plot(tVec, squeeze(sum(popMat(r, ...
            ...
            getColour(r, nReefs), 'lineWidth', 1.25)
    end
    xlim([0, max(tVec) + 1])
