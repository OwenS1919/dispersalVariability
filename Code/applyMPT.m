function [portVec, portRet, portVar, boundVals, exitFlag] = applyMPT(data, ...
    weight, discreteInd, resources, costs, boundVals, calcBoundValsInd, ...
    useParallelFlag)
% applyMPT() is a function which will apply modern portfolio theory to a
% dataset and return the optimal portfolio configuration
...
% note that for the continuous case, we are assuming that all investment
% funds must be spent

% note - since I've made some changes to the boundary values stuff, the
% continuous implementation may not work lol
...
% weight - optional - a weight parameter representing the trade - off
    % between the expected return and variance for the whole portfolio - in
    % this case, both expected return and variance will be scaled relative
    % to the max values in each, and weight = 1 will indicate only the
    % expected return is to be prioritised - default is 0.8
...
% portRet - a scalar indicating the optimal portfolio's expected return,
    % i.e. the expected return under portVec
% portVar - a scalar indicating the optimal portfolio's variance under
    % portVec
...
if nargin < 3 || isempty(discreteInd)
    discreteInd = "continuous";
end
if nargin < 4 || isempty(resources)
    resources = round(nAssets / 3);
end
if nargin < 5 || isempty(costs)
    costs = ones(1, nAssets);
end
if nargin < 6 || isempty(boundVals)
    boundVals = [0, 1; -1, 0];
end
if nargin < 7 || isempty(calcBoundValsInd) || calcBoundValsInd ...
        ~= "calcBoundVals"
    calcBoundVals = false;
else
    calcBoundVals = true;
end
if nargin < 8 || isempty(useParallelFlag)
    useParallelFlag = "no";
end
...
% will be using fmincon() which treats x as the same orientation as x0 a
% for the continuous optimisation case, and for now I'm just going to use
% the ga() function which accepts a row vector for the integer constrained
% version, however if we're having any issues then the surrogateopt()
% function may be a good alternate approach
